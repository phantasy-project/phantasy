...
It is developed to assist the channel finder service, but could be used for other purpose.

It supports 2 different CSV file formats.

Format 1, which starts with pv name as first column, following by properties and tags.
It supports 2 concept with property and tag.
Each cell except the first one is a property name value pair if it has format like A=B with property name "A"
and property value "B". Otherwise it is a tag.
Example:
    PV,machine=xxx,elemIndex=xxx,elemPosition=xxx,elemName=xxx,elemHandle=xxx,elemField=xxx,elemType=xxx,tag1,tag2

Format 2: Each column has a head label is a property except for the column "PV". Otherwise, it is a tag.
Example:
...
from phylib_err import CSVFormatError
...
def __read_csv_1(csvdata):
    ...
    keys = csvdata[0]
    # lower all headers
    keylow = [label.lower() for label in keys]
    pv_idx = keylow.index('pv')
