...
from phyhlc import catools
from phyhlc.cothread import Popen
from phyutil.phylib.cfg import Configuration
from phyutil.phylib.layout.accel import SeqElement
...
from phyutil.phylib.lattice.flame import FlameLatticeFactory
...
def build_virtaccel(layout, **kwargs):
    """Convenience method to build a virtual accelerator.
       ...
       :param start: name of accelerator element to start simulation
       :param end: name of accelerator element to end simulation
       :param data_dir: path of directory containing FLAME data files
       :param work_dir: path of directory for execution of FLAME
...
class VirtualAccelerator(object):
    """VirtualAccelerator executes and manages the
       EPICS IOC process and FLAME simulation thread.
    """
    ...
    def _co_execute(self):
        ...
        while self._continue:
            ...
            if machine is None:
                ...
                machine = flame.Machine(lattice.conf())
            else:
